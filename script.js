...
window.addEventListener('scroll', () => {
  if (window.scrollY > 60) {
    nav.style.padding    = '.65rem 3.5rem';
    nav.style.boxShadow  = '0 4px 30px rgba(184,146,42,.1)';
    nav.style.background = 'rgba(255,255,255,.97)';
  } else {
    nav.style.padding    = '1.1rem 3.5rem';
...
// ── 20. "GET STARTED" SCROLL BUTTONS ─────
// These are plain <button> elements (not links), so clicking them scrolls
// smoothly to the target section without changing the page URL/hash.
